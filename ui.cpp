...
void Interface::displayitemslot(int slotno, Item & it, int left) {
	int attrs = (it.active ? COLOR_PAIR(COLOR_CYAN ^ 7) | A_BOLD
	...
	displayitembar(slotno, it.status, it.active);
}
...
Command Interface::detailspage(Being & b) {
    ...
    do {
        Being & viewed = (viewingplayer ? BEING[0] : b);
        displaydetails(viewed);
        if (viewed.isplayer) {
            if (b.isplayer)
                UI.msg("Viewing your system details. Press any key...");
            else {
                char msgbuf[128];

                strcpy(msgbuf, "Viewing your system details. Press d to view ");
...
void Interface::displayhelp() {
	...
	attron(attrs);
	mvaddstr(3, 9, "Controls:");
	attroff(attrs);
	attrs = COLOR_PAIR((COLOR_BLUE ^ 7) | (COLOR_BLUE << 3));
	attron(attrs);
	addstr("_____________________");
	attroff(attrs);
	windowline( 4, "movement:                 ");
	windowline( 5, "                          ");
	windowline( 6, "7 8 9      y k u          ");
    windowline( 7, " \\|/        \\|/           ");
	windowline( 8, "4-@-6  or  h-@-l          ");
    windowline( 9, " /|\\        /|\\           ");
	windowline(10, "1 2 3      b j n          ");
	windowline(11, "                          ");
	windowline(12, "1-8: use system 1-8       ");
	windowline(13, "  .: wait                 ");
    windowline(14, "  d: view system details  ");
