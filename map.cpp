...
void Map::rect(int t, int b, int l, int r, TileType interior, TileType border) {
	int i, j;
...
void Map::display(int x, int y) {
	...
	if (tile[x][y].seen & TILEFLAG_SEEN) {
		tile[x][y].getappearance(symbol, fore, back);
		if (!(tile[x][y].seen & TILEFLAG_INLOS)) {
			 fore = COLOR_WHITE;
			 back = COLOR_BLACK;
		}
...
void Map::visitLOS(int centerx, int centery, int radius, int terrain,
	...
	for (i = 0; i < MAPHEIGHT; i++)
		for (j = 0; j < MAPWIDTH; j++)
			if (checkLOS(centerx, centery, radius, terrain, i, j))
				(this->*visit)(i, j);			
				
	return; */ // for sanity-checking checkLOS
